<script lang="ts">
	...
	import { useSelectedWallet } from '$lib/composables/wallet.svelte';
	...
	function handleSelectWallet(wallet: WalletName) {
		useSelectedWallet.selectedWallet = getWallet(wallet);
	}
</script>

<div class="relative w-fit">
	<button class="flex cursor-pointer flex-row" onclick={() => (isOpen = !isOpen)}>
		{useSelectedWallet.selectedWallet.name}
		<img
			alt={useSelectedWallet.selectedWallet.name}
			src={useSelectedWallet.selectedWallet.icon}
			width="24px"
		/>
	...
	{#if isOpen}
		<ul transition:slide class="absolute flex w-auto flex-col">
			{#each wallets as wallet}
				<li
					class={`${useSelectedWallet.selectedWallet.name === wallet ? 'bg-primary-500/30' : 'bg-primary-500/10'} 
				...
				>
					<button class="cursor-pointer" onclick={() => handleSelectWallet(wallet)}>
						{wallet}
					</button>

					<CheckIcon
						class={`${useSelectedWallet.selectedWallet.name === wallet ? 'opacity-100' : 'opacity-0'}`}
					/>
				</li>
			{/each}
		</ul>
